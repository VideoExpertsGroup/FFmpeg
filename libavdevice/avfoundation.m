...
typedef struct
{
    ...
    int		   memcpy_offset; //<binary> change
    int		   is_landscape; //<binary> modification
         ...
         fromConnection:(AVCaptureConnection *)connection
{
    /*<binary>*/
    if (connection) {
	if (_context->is_landscape) {
//	    av_log(_context, AV_LOG_INFO, "<binary>ORIENTATION LANDSCAPE\n");
	    [connection setVideoOrientation: AVCaptureVideoOrientationLandscapeRight ];
	} else {
//	    av_log(_context, AV_LOG_INFO, "<binary>ORIENTATION PORTRAIT\n");	
...
//        [video_device setValue:min_frame_duration forKey:@"activeVideoMaxFrameDuration"];
	...
	[video_device setActiveVideoMinFrameDuration: CMTimeMake(1, framerate)]; 
	    ...
	    [video_device setActiveColorSpace: AVCaptureColorSpace_sRGB];
	}
    } else {
...
static int get_video_config(AVFormatContext *s)
{
	...
	stream->codecpar->height     = (int)image_buffer_size.height;
        ...
        stream->codecpar->width      = (int)image_buffer_size.height;
	stream->codecpar->height     = (int)image_buffer_size.width;
    }
    /*</binary>*/
...
static int avf_read_header(AVFormatContext *s)
{
    ...
    if (@available(iOS 10.2, *)) {
        AVCaptureDeviceDiscoverySession *session= [AVCaptureDeviceDiscoverySession
                                                   discoverySessionWithDeviceTypes: @[AVCaptureDeviceTypeBuiltInDualCamera,AVCaptureDeviceTypeBuiltInWideAngleCamera]
    ...
    if (ctx->video_device_index >= 0) {
        ...
        if (!strncmp(ctx->video_filename, "default", 7)) {
    		...
    		video_device = session.devices.firstObject;
	    } else {
    ...
    //<binary> set color to srgb to prevent set p3-colorspace
    if (@available(iOS 10.2, *)) {
	[ctx->capture_session setAutomaticallyConfiguresCaptureDeviceForWideColor: NO];
    } 
...
static int avf_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    do {
        ...
        if (ctx->current_frame != nil) {
            void *data;
            if (av_new_packet(pkt, (int)CVPixelBufferGetDataSize(image_buffer) + 2048) < 0) {
                return AVERROR(EIO);
            }
	    ...
	    size_t bpr = 0;
	        ...
	        void* plane_data;
	        size_t w, h;
		    h 	= CVPixelBufferGetHeight(image_buffer);	        
		    w 	= CVPixelBufferGetWidth (image_buffer);
	        //av_log(s, AV_LOG_INFO, "<binary> plane buffer(%p) %zu x %zu with %zu planes  \n", data, w, h, plane_count);
	        for(size_t i = 0; i< plane_count; i++){
		    h 	= CVPixelBufferGetHeightOfPlane(image_buffer, i);	        
		    w 	= CVPixelBufferGetWidthOfPlane(image_buffer, i);
		    bpr = CVPixelBufferGetBytesPerRowOfPlane(image_buffer, i);
		    plane_data = CVPixelBufferGetBaseAddressOfPlane(image_buffer, i);
		    //av_log(s, AV_LOG_INFO, "<binary> plane buffer %zu wxh: %zu x %zu; bpr %zu; (%p) \n", i, w, h, bpr, plane_data);    
	        }
	    } else {
	        //av_log(s, AV_LOG_INFO, "<binary> non-plane buffer\n");
	    }
	    
	    //av_packet_add_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, (uint8_t*) &bpr, sizeof(size_t) );
	    //uint8_t* add = av_packet_new_side_data(pkt, AV_PKT_DATA_NEW_EXTRADATA, sizeof(size_t));
	    //memcpy(add, bpr, sizeof(bpr));
	    pkt->convergence_duration = bpr; //use deprecated value

            if (ctx->memcpy_offset) { //<binary> experiment
                memcpy(pkt->data, data + ctx->memcpy_offset, pkt->size - ctx->memcpy_offset );
            } else {
...
static const AVOption options[] = {
    ...
    { "memcpy_offset", "<binary> experiment: avpkt memcpy offset", offsetof(AVFContext, memcpy_offset), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, AV_OPT_FLAG_DECODING_PARAM },
    { "is_landscape", "get frames in landscape or portrait resolution", offsetof(AVFContext, is_landscape), AV_OPT_TYPE_INT, {.i64=1}, 0, 1, AV_OPT_FLAG_DECODING_PARAM, "is_landscape" },
    { "true",  "", 0, AV_OPT_TYPE_CONST, {.i64=1}, 0, 0, AV_OPT_FLAG_DECODING_PARAM, "is_landscape" },
    { "false", "", 0, AV_OPT_TYPE_CONST, {.i64=0}, 0, 0, AV_OPT_FLAG_DECODING_PARAM, "is_landscape" },
