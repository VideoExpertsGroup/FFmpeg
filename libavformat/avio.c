...
int ffurl_connect(URLContext *uc, AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_WARNING, "ffurl_connect: protocol name %s, options %p\n", uc->prot->name, options);
...
int ffurl_open(URLContext **puc, const char *filename, int flags,
               const AVIOInterruptCB *int_cb, AVDictionary **options)
{
    av_log(NULL, AV_LOG_WARNING, "ffurl_open: before options %p, %p\n", options, ((options != NULL) ? (*options) : NULL));
                                                              ...
                                                              int size))
{
    ...
    while (len < size_min) {
        ...
        av_log(NULL, AV_LOG_TRACE, "retry_transfer_wrapper: transfer_func for proto %s, size %d, len %d, (size - len) %d\n", h->prot->name, size, len, (size - len));
        ret = transfer_func(h, buf + len, size - len);
        av_log(NULL, AV_LOG_TRACE, "retry_transfer_wrapper: transfer_func return %d\n", ret);
