...
static int open_input(HLSContext *c, struct playlist *pls, struct segment *seg)
{
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        if (strlen(c->ssl_key) > 0 )
            av_strlcpy(key, c->ssl_key, FFMIN(sizeof(key), strlen(c->ssl_key)));
...
{
    AVDictionaryEntry *ssl_key_dict = NULL;
    AVDictionaryEntry *start_offset_dict = NULL;
    AVDictionaryEntry *start_path_dict = NULL;
    AVDictionaryEntry *ext_stream_dict = NULL;
    AVDictionaryEntry *start_cookies_dict = NULL;
    ...
    ssl_key_dict = av_dict_get(s->metadata, "ssl-key", NULL, 0);
    if (ssl_key_dict != NULL && ssl_key_dict->value != NULL && strlen(ssl_key_dict->value) > 0)
    {
        ...
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: sslkey %s(%s)\n", ssl_key_dict->value, c->ssl_key);
    }

    // get start_offset form dict
    start_offset_dict = av_dict_get(s->metadata, "start-offset", NULL, 0);
    if (start_offset_dict != NULL && start_offset_dict->value != NULL && strlen(start_offset_dict->value) > 0)
    {
        c->start_offset = strtoll(start_offset_dict->value, NULL, 10);
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: start_offset %lld\n", c->start_offset);
    }

    // get start_path form dict
    memset(c->start_path, 0, MAX_URL_SIZE);
    start_path_dict = av_dict_get(s->metadata, "start-path", NULL, 0);
    if (start_path_dict != NULL && start_path_dict->value != NULL && strlen(start_path_dict->value) > 0)
    {
        ...
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: start_path %s(%s)\n", start_path_dict->value, c->start_path);
    }

    // get start_path form dict
    c->ext_stream  = -1;
    ext_stream_dict = av_dict_get(s->metadata, "ext-stream", NULL, 0);
    if (ext_stream_dict != NULL && ext_stream_dict->value != NULL && strlen(ext_stream_dict->value) > 0)
    {
        c->ext_stream = strtoll(ext_stream_dict->value, NULL, 10);
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: ext_stream %d\n", c->ext_stream);
    }

    // get start_cookies form dict
    memset(c->start_cookies, 0, 4096);
    start_cookies_dict = av_dict_get(s->metadata, "start-cookies", NULL, 0);
    if (start_cookies_dict != NULL && start_cookies_dict->value != NULL && strlen(start_cookies_dict->value) > 0)
    {
        ...
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: start_cookies: %s(%s)\n", start_cookies_dict->value, c->start_cookies);
        av_log(NULL , AV_LOG_WARNING, "hls_read_header: final cookies: %s\n", c->cookies);
    }

    av_log(NULL , AV_LOG_WARNING, "hls_read_header: file name %s\n",s->filename);
...
static const AVOption hls_options[] = {
    {"ssl-key", "set ssl key", OFFSET(opt_ssl_key), AV_OPT_TYPE_STRING, {.str = ""},  0, 0, FLAGS, "ssl-key"},
    {"start-offset", "set start_offset", OFFSET(start_offset), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT64_MAX, FLAGS, "start-offset"},
    {"start-path", "set start path", OFFSET(opt_start_path), AV_OPT_TYPE_STRING, {.str = ""},  0, 0, FLAGS, "start-path"},
    {"start-cookies", "set start cookies", OFFSET(opt_start_cookies), AV_OPT_TYPE_STRING, {.str = ""},  0, 0, FLAGS, "start-cookies"},
