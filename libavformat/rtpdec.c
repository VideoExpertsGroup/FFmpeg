...
{
    int payload_len;
    AVFormatContext* avctx = s->ic;
    RTSPState* rstate = NULL;
    
    if(avctx)
        rstate = avctx->priv_data;

    if (rstate)
    {

        int video_idx = 0;
        for (int n = 0; n < s->ic->nb_streams; n++)
        {
            if (s->ic->streams[n]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
            {
                video_idx = s->ic->streams[n]->index;
            }
        }

        if (s->st->index == video_idx)
        {
            if (rstate->rtcp_sr_data_size && rstate->rtcp_sr_ptr)
            {
                if (RTCP_SR == buf[1])
                {
                    *rstate->rtcp_sr_data_size = 0;
                    if (rstate->rtcp_sr_buf_size < len)
                    {
                        *rstate->rtcp_sr_data_size = -1;
                    }
                    else
                    {
                        memcpy((unsigned char*)rstate->rtcp_sr_ptr, buf, len);
                        *rstate->rtcp_sr_data_size = len;
                        av_log(s->ic, AV_LOG_TRACE, "received %d bytes of SR\n", len);
                    }
                }
            }
        }
                                  ...
                                  AVIOContext *avio, int count, int prot /*TCP - 0, UDP - 1*/)
{
    ...
    if ((len > 0) && buf) {
        ...
        if (rstate)
        {
            ...
            if (s->st->index == video_idx)
            {
                if (rstate->rtcp_rr_data_size && rstate->rtcp_rr_ptr)
                {
                    *rstate->rtcp_rr_data_size = 0;
                    if (rstate->rtcp_rr_buf_size < len)
                    {
                        *rstate->rtcp_rr_data_size = -1;
                    }
                    else
                    {
                        memcpy((unsigned char*)rstate->rtcp_rr_ptr, buf, len);
                        *rstate->rtcp_rr_data_size = len;
                    }
                }
            }
        }
                                     ...
                                     const uint8_t *buf, int len)
{
    ...
    s->ssrc = ssrc;
	 ...
	 if (s->cur_timestamp == 0) 
	 	 av_log(s->ic, AV_LOG_ERROR,
			  "RTP: PT=%02x: cseq:%04x timestamp:%u ssrc:%u\n",
			  payload_type, seq, timestamp, ssrc );
