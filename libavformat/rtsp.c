...
const AVOption ff_rtsp_options[] = {
    ...
    { "reply_text_ptr", "Pointer to server reply in text", OFFSET(reply_text_ptr), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC|ENC },
    { "reply_code_ptr", "Pointer to server reply in int" , OFFSET(reply_code_ptr), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC|ENC },
    { "rtcp_first_package", "FIRST RTCP package", OFFSET(rtcp_first_package), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, AV_OPT_FLAG_EXPORT },
    { "first_rtp_pts", "timestamp in first RTP packet", OFFSET(first_pts), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, AV_OPT_FLAG_EXPORT},
    { "first_pts_on_play", "start range in RTSP header after PLAY", OFFSET(first_pts_on_play), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, AV_OPT_FLAG_EXPORT},
    { "rtcp_rr_ptr", "RTCP_RR buffer", OFFSET(rtcp_rr_ptr), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "rtcp_rr_buf_size", "Size of RTCP_RR buffer", OFFSET(rtcp_rr_buf_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "rtcp_rr_data_size", "Size of RTCP_RR data", OFFSET(rtcp_rr_data_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "rtcp_sr_ptr", "RTCP_SR buffer", OFFSET(rtcp_sr_ptr), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "rtcp_sr_buf_size", "Size of RTCP_SR buffer", OFFSET(rtcp_sr_buf_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "rtcp_sr_data_size", "Size of RTCP_SR data", OFFSET(rtcp_sr_data_size), AV_OPT_TYPE_INT64,{ .i64 = 0 }, INT64_MIN, INT64_MAX, DEC | ENC },
    { "workaround_disable_firstpos_check_rtp_packet_magic_dollar",  "disable check on first position in buffer for rtp packet boundary magic dollar. it was such before ver 2.8.",
        OFFSET(workaround_disable_firstpos_check_rtp_packet_magic_dollar), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, DEC },
    { NULL },
};
...
#if 0
	s->encrypted = strstr(content, "encrypted=1");
	if(s->encrypted)
		av_log(NULL, AV_LOG_DEBUG, "Encrypted\n");
#endif
...
void ff_rtsp_close_streams(AVFormatContext *s)
{
    ...
    av_log(s, AV_LOG_TRACE, "ff_rtsp_close_streams rtcp_first_package:%p ",rt->rtcp_first_package);
    if(rt->rtcp_first_package != NULL) {av_free(rt->rtcp_first_package); rt->rtcp_first_package = NULL;}

    av_log(NULL, AV_LOG_TRACE, "ff_rtsp_close_streams 1\n");
    ...
    av_log(NULL, AV_LOG_TRACE, "ff_rtsp_close_streams 2\n");
    ...
    av_log(NULL, AV_LOG_TRACE, "ff_rtsp_close_streams exit\n");
...
static void rtsp_parse_range(int *min_ptr, int *max_ptr, const char **pp)
{
    ...
    v = strtol(q, &p, 10);
    //av_log(NULL, AV_LOG_DEBUG, "rtsp_parse_range min:%d (%s)\n", v,q);
    if (*p == '-') {
        p++;
        *min_ptr = v&0x7FFFFFFF;
        v = strtol(p, &p, 10);
        //av_log(NULL, AV_LOG_DEBUG, "rtsp_parse_range max:%d (%s)\n", v,p);
        *max_ptr = v&0x7FFFFFFF;
    } else {
        *min_ptr = v&0x7FFFFFFF;
        *max_ptr = v&0x7FFFFFFF;
    }
...
start:
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            av_log(s, AV_LOG_TRACE, "ret=%d c=%02x [%c], workaround_disable_firstpos_check_rtp_packet_magic_dollar: %d\n", ret, ch, ch, rt->workaround_disable_firstpos_check_rtp_packet_magic_dollar);
            if (ret != 1)
                return AVERROR_EOF;
            if (ch == '\n')
                break;
            if ((ch == '$' && rt->workaround_disable_firstpos_check_rtp_packet_magic_dollar == 0 && q == buf) ||
                    (ch == '$' && rt->workaround_disable_firstpos_check_rtp_packet_magic_dollar != 0))
            {
        ...
        *q = '\0';
        ...
        if(rt->reply_text)
        {
            av_strlcat(rt->reply_text, p, MAX_REPLY_LENGTH);
            av_strlcat(rt->reply_text, "\n", MAX_REPLY_LENGTH);
        }
        ...
        if (line_count == 0) {
            ...
            if (!strncmp(buf1, "RTSP/", 5)) {
                ...
                {
                    rt->reply_code[0] = 0;
                    av_strlcatf(rt->reply_code, MAX_REPLY_LENGTH, "%d", (int)reply->status_code);
                }
...
int ff_rtsp_connect(AVFormatContext *s)
{
	...
	int lower_http_transport_mask = 0;
    ...
    rt->first_pts = -1;
    rt->first_pts_on_play = -1;
    rt->rtcp_first_package_fill = -1;
    rt->rtcp_first_package = av_malloc(1024);

    av_log(s, AV_LOG_ERROR, "ff_rtsp_connect rtcp_first_package:%p ",rt->rtcp_first_package);
    av_log(s, AV_LOG_ERROR, "ff_rtsp_connect  %lli:%lli", rt->first_pts, rt->first_pts_on_play);

    if (s->max_delay < 0) /* Not set by the caller */
        s->max_delay = s->iformat ? DEFAULT_REORDERING_DELAY : 0;

	 lower_http_transport_mask = rt->lower_transport_mask;


    rt->control_transport = RTSP_MODE_PLAIN;
    if (rt->lower_transport_mask & (1 << RTSP_LOWER_TRANSPORT_HTTP) || rt->lower_transport_mask & (1 << RTSP_LOWER_TRANSPORT_HTTPS)) {
...
redirect:
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        ...
        char headers[1024];
		  ...
		  av_log(s, AV_LOG_WARNING, "Protocol2 %d:(%s)",lower_http_transport_mask,(lower_http_transport_mask & (1 << RTSP_LOWER_TRANSPORT_HTTP)) ? "http" : "https");
		  
        ff_url_join(httpname, sizeof(httpname), (lower_http_transport_mask & (1 << RTSP_LOWER_TRANSPORT_HTTP)) ? "http" : "https", ""/*auth*/, host, port, "%s", path);
...
//    r3355 | Sergey | 2017-03-15 16:54:31 +0700 (Wed, 15 Mar 2017) | 1 line
//    ffmpeg.3.2.4 - fixed https rtsp tunneling.
    if (tcp_fd < 0) {
        //err = tcp_fd;
        //goto fail;
        av_log(s, AV_LOG_ERROR, "ff_rtsp_connect:  ffurl_get_file_handle return %d, skip fail for tunneling", tcp_fd);
    }
...
redo:
    ...
    case RTSP_LOWER_TRANSPORT_TCP:
        len = ff_rtsp_tcp_read_packet(s, &rtsp_st, rt->recvbuf, RECVBUF_SIZE);
	  	  ...
	  	  av_log(NULL, AV_LOG_INFO, "ff_rtsp_fetch_packet transport:%d\n",rt->transport);
        if (len > 0 && rtsp_st->transport_priv && rt->transport == RTSP_TRANSPORT_RTP)
        {
         //   ff_rtp_check_and_send_back_rr(rtsp_st->transport_priv, rtsp_st->rtp_handle, NULL, len);
		        ff_rtp_check_and_send_back_rr(rtsp_st->transport_priv, rt->rtsp_hd, NULL, len,0/*TCP*/);
        }    
        #endif
    ...
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
        ...
        if (len > 0 && rtsp_st->transport_priv && rt->transport == RTSP_TRANSPORT_RTP)
            ff_rtp_check_and_send_back_rr(rtsp_st->transport_priv, rtsp_st->rtp_handle, NULL, len,1/*UDP*/);
        break;
    case RTSP_LOWER_TRANSPORT_CUSTOM:
        ...
        if (len > 0 && rtsp_st->transport_priv && rt->transport == RTSP_TRANSPORT_RTP)
            ff_rtp_check_and_send_back_rr(rtsp_st->transport_priv, NULL, s->pb, len , 1/*UDP*/);
        break;
    }
