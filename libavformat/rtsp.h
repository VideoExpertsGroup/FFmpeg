...
typedef struct RTSPState {
	...
	// FIRST RTP timestamps from RTP and RTSP header .
	// Addinial optional for customer request
	int64_t first_pts;
	int64_t first_pts_on_play;
	// RTSP paramaters
	char* rtcp_first_package;
	int rtcp_first_package_fill;

    int64_t rtcp_rr_ptr;
    int64_t rtcp_rr_buf_size;
    int64_t* rtcp_rr_data_size;

    int64_t rtcp_sr_ptr;
    int64_t rtcp_sr_buf_size;
    int64_t* rtcp_sr_data_size;
    
    /**
     * This is workaround for disable check on first position in buffer for rtp packet boundary magic dollar.
     * It was such before ver 2.8.
     * For trying fix high CPU usage on a RTSP streams with garbadge after RTP packet
     */
    int workaround_disable_firstpos_check_rtp_packet_magic_dollar;
