...
static int rtsp_read_play(AVFormatContext *s)
{
    ...
    if (!(rt->server_type == RTSP_SERVER_REAL && rt->need_subscription)) {
        ...
        if (rt->state == RTSP_STATE_PAUSED) {
            ...
            //rt->seek_timestamp = 1501180491;//20170727T183451Z
            //rt->seek_timestamp = 1503061592;//20170818T130632Z
            if (rt->seek_timestamp)
            {
                ...
                timeinfo->tm_sec   = 32;
                */
                strftime(sz_time, sizeof(sz_time), "Range: clock=%Y%m%dT%H%M%SZ\r\n", timeinfo);
                snprintf(cmd, sizeof(cmd), "Require: onvif-replay\r\n%s", sz_time);
//              time_t ttime = mktime(timeinfo) - _timezone;
//              av_log(s, AV_LOG_DEBUG, "ttime=%d\n", ttime);
            }
#else
...
#if 0
                if (rtpctx->range_start_offset != 0) {
                    rt->first_pts_on_play = rtpctx->range_start_offset;
                }
                av_log(s, AV_LOG_ERROR, "range_start_offset: %lld:%lld\n", rtpctx->range_start_offset,rt->first_pts_on_play);
#endif                
            }
        }
    }
...
#if 0    
    if (rt->first_pts == -1 || rt->rtcp_first_package_fill == -1) {
        int ii;
        for (ii = 0; ii < rt->nb_rtsp_streams; ii++) {
            RTSPStream *rtsp_st = rt->rtsp_streams[ii];
            RTPDemuxContext *rtpctx = rtsp_st->transport_priv;
            //if (s->streams[ii]->->codec->codec_type == MEDIA_TYPE_VIDEO) 
            {
                //rtpctx->last_rtcp_ntp_time  = AV_NOPTS_VALUE;
                //rtpctx->first_rtcp_ntp_time = AV_NOPTS_VALUE;
                //rtpctx->base_timestamp 	  = 0;
                ...
                //rtpctx->unwrapped_timestamp = 0;
                //rtpctx->rtcp_ts_offset 	  = 0;
                if (rt->rtcp_first_package_fill == -1 && rtpctx->last_rtcp_ntp_time  != AV_NOPTS_VALUE) {
                    rt->rtcp_first_package[0] = 0;
                    //rt->rtcp_first_package = av_strdup("1111111");
                    //av_strlcat(rt->rtcp_first_package,"TESTEST", sizeof(rt->last_subscription)); 
                    av_strlcatf(rt->rtcp_first_package, 1024, "first_rtcp_ntp_time:%llu;last_rtcp_ntp_time:%llu;last_rtcp_timestamp:%u;",
                                    rtpctx->first_rtcp_ntp_time,rtpctx->last_rtcp_ntp_time, rtpctx->last_rtcp_timestamp);
                    av_log(s, AV_LOG_ERROR, "rtp_read_play1 rtcp_first_package(%s) \n",rt->rtcp_first_package);
                    rt->rtcp_first_package_fill = 1;
                }
                else if (rt->first_pts == -1 && rtpctx->cur_timestamp != 0) {
                    rt->first_pts =  (uint64_t)rtpctx->cur_timestamp;
                    av_log(s, AV_LOG_ERROR, "rtp_read_play1 type:%d %u:%u:%u:%u\n",rtpctx->payload_type, rtpctx->cur_timestamp, rtpctx->base_timestamp,rtpctx->timestamp,rtpctx->last_rtcp_timestamp);
                    av_log(s, AV_LOG_ERROR, "rtp_read_play1 first_pts:%lli\n",rt->first_pts);
                }
            }
        }
    }
                          ...
                          int64_t timestamp, int flags)
{
    ...
    //av_log(NULL, AV_LOG_DEBUG, "rtsp_read_seek %llu\n", timestamp);
