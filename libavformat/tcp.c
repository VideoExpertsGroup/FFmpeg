...
static int tcp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (!hostname[0])
        ret = ff_getaddrinfo_a(h, NULL, portstr, &hints, &ai);
    else
        ret = ff_getaddrinfo_a(h, hostname, portstr, &hints, &ai);
#else
    if (!hostname[0])
...
{
    av_log(h, AV_LOG_DEBUG, "tcp_open2: options %p\n", options);
