...
static int udp_open2(URLContext *h, const char *uri, int flags, AVDictionary** options)
{
    av_log(h, AV_LOG_DEBUG, "udp_open2: options %p\n", options);
    if (h && h->priv_data && options)
    {
        AVDictionary *tmp = NULL;
        UDPContext *s = h->priv_data;
        av_dict_copy(&tmp, *options, 0);

        int ret = av_opt_set_dict(s, &tmp);
        av_log(h, AV_LOG_DEBUG, "udp_open2: av_opt_set_dict return %d\n", ret);

        av_dict_free(&tmp);
        // AVDictionaryEntry* key_buffer_size = av_dict_get(*options, "buffer_size", NULL, 0);
        // av_log(h, AV_LOG_DEBUG, "udp_open2: key_buffer_size %p\n", key_buffer_size);
        // if (key_buffer_size != NULL && key_buffer_size->value != NULL)
        // {
        //     s->buffer_size = strtol(key_buffer_size->value, NULL, 10);
        //     av_log(h, AV_LOG_DEBUG, "udp_open2: buffer_size %d\n", s->buffer_size);
        // }
    }
