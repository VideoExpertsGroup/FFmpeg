...
int av_opt_set(void *obj, const char *name, const char *val, int search_flags)
{
    ...
    if (!o || !target_obj)
    {
        av_log(obj, AV_LOG_ERROR, "av_opt_set: Option not found -> name \"%s\", value \"%s\" \n", name, val);
    ...
    {
        av_log(obj, AV_LOG_ERROR, "av_opt_set: val == NULL impossible!\n");
        return AVERROR(EINVAL);
    }

    if (o->flags & AV_OPT_FLAG_READONLY)
    {
        av_log(obj, AV_LOG_ERROR, "av_opt_set: flags is AV_OPT_FLAG_READONLY!\n");
        return AVERROR(EINVAL);
    }
    ...
    av_log(obj, AV_LOG_ERROR, "av_opt_set: invalid set -> name %s, value %s\n", name, val);
    return AVERROR(EINVAL);
}
...
int av_opt_set_dict2(void *obj, AVDictionary **options, int search_flags)
{
    ...
    while ((t = av_dict_get(*options, "", t, AV_DICT_IGNORE_SUFFIX))) {
		av_log(obj, AV_LOG_WARNING, "=av_opt_set_dict2 option %s to value %s.\n", t->key, t->value);
